...
- (void)viewDidLoad
{
   ...
   // ACAccountStore *accountStore = [[ACAccountStore alloc] init];
    ...
    self.identifier = _selectAccount.identifier;
    ...
    ^(BOOL granted, NSError *error) {
        ...
        if (granted) {
          ...
          //  self.twitterAccounts = [self.accountStore accountsWithAccountType:twitterType];
            ...
            _selectAccount = [_accountStore accountWithIdentifier:self.identifier];
            ...
            [request setAccount:_selectAccount];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    //subviewのimageSelectはtweetviewに表示するためだけ
    //imageは実際に投稿してる画像
    _image = [info objectForKey:UIImagePickerControllerOriginalImage];
   // _image = _userimage;
    
    _imageSelect = [[UIImageView alloc] initWithImage:_image];
    _imageSelect.frame = CGRectMake(265, 5, 48, 48);
    
    //[[UIImageView alloc] initWithImage:_image].frame = CGRectMake(265, 5, 48, 48);
    
    [self.view addSubview:_imageSelect];
    _imageSelect.layer.masksToBounds = YES; //imageViewの角丸
    _imageSelect.layer.cornerRadius = 7.0; //imageViewの角丸
